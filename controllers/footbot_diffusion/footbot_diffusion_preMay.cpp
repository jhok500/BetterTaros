...
void CFootBotDiffusion::Init(TConfigurationNode& t_node) {
   ...
   GetNodeAttributeOrDefault(t_node, "velocity", m_fWheelVelocity, m_fWheelVelocity);
    ...
    YawSensor = new boost::circular_buffer<double>(2*RobotNumber*2);
...
void CFootBotDiffusion::ControlStep() {
    ...
    for(CCI_RangeAndBearingSensor::SPacket packet : packets)
    {
        ...
        double bearing = ToDegrees(packet.HorizontalBearing).GetValue();
        if (Time >501 && ID == 83 && abs(bearing) > 945) {
    ...
    if (IntCoord->size() == IntCoord->capacity()) {
        ...
        YawHolder->push_back(atan2(((IntCoord->at(Indices.front() + 1)) - (IntCoord->at(Indices.front() + 1 + (3*RobotNumber)))),
                                   ((IntCoord->at(Indices.front() + 2)) -
                                    (IntCoord->at(Indices.front() + 2 + (3*RobotNumber))))));
